if __name__ == '__main__':
    ...
    # q_g_dist = np.dot(mquery_feature, np.transpose(gallery_feature))
    q_q_dist = np.dot(query_feature, np.transpose(query_feature))
    # q_q_dist = np.dot(mquery_feature, np.transpose(mquery_feature))
    ...
    for i in range(len(query_label)):
        ...
        # ap_tmp, CMC_tmp = evaluate(re_rank[i, :], mquery_label[i], mquery_cam[i],
                                   # gallery_label, gallery_cam)
        if CMC_tmp[0] == -1:
            continue
        CMC = CMC + CMC_tmp
    ...
    CMC_np = CMC.numpy()
    fig = plt.figure()
    plt.plot(np.arange(20), CMC_np[0:20], 'ro-')
    fig.savefig(os.path.join('./model', 'CMC_reranking.jpg'))
