...
def evaluating(qf, ql, qc, gf, gl, gc):
    ...
    # Same identities from different cameras (good index)
    good_index = np.setdiff1d(query_index, camera_index, assume_unique=True)
    # Same identities from same cameras (bad index)
    bad_index = np.intersect1d(query_index, camera_index)
...
result = scipy.io.loadmat('features_duke_60.mat')
...
ax.set(xlabel='Rank', ylabel='Matching Accuracy (%)', title='CMC Curve on DukeMTMC')
ax.grid(linestyle='-.', color='b', linewidth=1)
fig.savefig(os.path.join('./DukeMTMC', 'CMC.jpg'))
