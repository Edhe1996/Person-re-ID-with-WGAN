...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='ResNet50_GAN', type=str, help='model name')
    parser.add_argument('--data_dir', default='/mnt/storage/scratch/xh17500/Dataset/Market1501_fake/pytorch', type=str,
                        help='training data path')
    parser.add_argument('--train_all', action='store_true', help='use all training data')
    parser.add_argument('--color_jitter', action='store_true', help='use color jitter in training')
    parser.add_argument('--batchsize', default=32, type=int, help='batch size')
    parser.add_argument('--epoch', default=40, type=int, help='number of epochs to train')
    ...
    def save_network(model, epoch):
        save_filename = 'network_gan_continue_%s.pth' % epoch
    ...
    def draw_curve(current_epoch):
        ...
        if current_epoch == 0:
            ax0.legend()
            ax1.legend()
        fig.savefig(os.path.join('./model', model_name, 'train_result_continue.jpg'))


    model = ResNet50Baseline(len(class_names))
    model.load_state_dict(torch.load('./model/ResNet50_GAN/network_last.pth'))
    # print(model)
    if use_gpu:
        model = model.cuda()
    ...
    # Train and validate for num_epoch epochs
